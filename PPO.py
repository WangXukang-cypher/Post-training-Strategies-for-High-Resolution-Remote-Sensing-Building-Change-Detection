# -*- coding: utf-8 -*-
# 运行方式：
# python retrainppo.py --data_root /fast/Wang/Chaofen/Smashiki_256QuaterCD --save_path /fast/Wang/Chaofen/S128exp/WHU-CD_iter_200000_lr_0.0002/retrained_ppo_best.pth
#
# PPO-ish 偏好微调：
# - 数据读取/预处理与 retrain.py 相同
# - 监督 + 边界/误检约束 + KL 保护
# - 偏好：基于 policy/ref logprob 比率的 PPO clip，优势来自 IoU 差的软版本
# - 轻量 EMA 用于评估/保存
...
# PPO 偏好
PPO_EPS        = 0.2
PPO_CLIP_VALUE = 5.0   # 限制比率爆炸
ADV_SOFT_K     = 6.0   # IoU 差 soft
LAMBDA_PPO     = 0.7

# EMA
EMA_DECAY = 0.999

# 只训练 decoder（如需更激进，置 True）
...
# ====== Dataset & Eval（与 retrain.py 保持一致）======
...
def build_eval_loader(args, split: str):
...
class BCDPairDataset(Dataset):
    """本地文件三件套读取：t1/t2/label，与 retrain.py 一致的 transform"""
...
# ---------- utils ----------
def as_prob(t: torch.Tensor) -> torch.Tensor:
    """兼容：如果输入已是[0,1]概率则直接用；否则当 logits 做 sigmoid"""
    if t.min() >= 0.0 and t.max() <= 1.0:
        return t.clamp(1e-6, 1 - 1e-6)
    return torch.sigmoid(t).clamp(1e-6, 1 - 1e-6)


def bernoulli_logprob(logits: torch.Tensor, mask01: torch.Tensor) -> torch.Tensor:
    """Bernoulli log p(x|logits), x in {0,1}"""
    log_p1 = -F.softplus(-logits)       # log(sigmoid)
    log_p0 = -logits + log_p1           # log(1-sigmoid)
    return mask01 * log_p1 + (1 - mask01) * log_p0


def kl_bernoulli(logits_p: torch.Tensor, logits_q: torch.Tensor) -> torch.Tensor:
    """KL(Bern(p) || Bern(q)) per-pixel"""
    p = as_prob(logits_p)
    q = as_prob(logits_q)
...
def boundary_loss(prob: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    """Sobel 边缘一致性"""
    sobel_x = torch.tensor([[1, 0, -1],
                            [2, 0, -2],
                            [1, 0, -1]], dtype=prob.dtype, device=prob.device)[None, None, ...]
...
def false_positive_penalty(prob: torch.Tensor, target: torch.Tensor, w=2.0) -> torch.Tensor:
    fp = prob * (1 - target)
    return (w * fp).mean()


def dice_loss(prob: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    return 1.0 - (2 * (prob * target).sum(dim=(1, 2, 3)) /
                  ((prob + target).sum(dim=(1, 2, 3)) + 1e-6)).mean()


def make_preference_masks(prob_policy: torch.Tensor,
                          prob_ref: torch.Tensor,
                          target: torch.Tensor,
                          th=0.5,
                          softness=ADV_SOFT_K):
    """
    构造 PPO-ish 偏好：
    - 把 policy/ref 概率二值化
    - 算 IoU 差作为 advantage（tanh soft）
    - 根据 adv 的正负，决定 pos/neg mask 取 policy 或 ref
    """
    ...
    adv = torch.tanh(softness * (iou_p - iou_r))  # [-1,1]
    prefer_policy = (adv >= 0).float()[:, None, None, None]

    m_pos = prefer_policy * p_mask + (1 - prefer_policy) * r_mask
    m_neg = prefer_policy * r_mask + (1 - prefer_policy) * p_mask

    return m_pos.detach(), m_neg.detach(), adv.detach()


def ppo_preference_loss(policy_logits: torch.Tensor,
                        ref_logits: torch.Tensor,
                        m_pos: torch.Tensor,
                        m_neg: torch.Tensor,
                        adv: torch.Tensor,
                        eps=PPO_EPS) -> torch.Tensor:
    """
    PPO-ish preference objective:
    ratio = exp( logp_policy - logp_ref ) ，再做 clip。
    注意：这里实现是“mask 既当标签又当选择区域”，与标准 PPO 不同（PPO-ish）。
    """
    logits_p_pos = policy_logits * m_pos
    logits_r_pos = ref_logits    * m_pos
    logits_p_neg = policy_logits * m_neg
    logits_r_neg = ref_logits    * m_neg

    lp_pos = bernoulli_logprob(logits_p_pos, m_pos).sum(dim=(1, 2, 3))
    lr_pos = bernoulli_logprob(logits_r_pos, m_pos).sum(dim=(1, 2, 3))
    lp_neg = bernoulli_logprob(logits_p_neg, m_neg).sum(dim=(1, 2, 3))
    lr_neg = bernoulli_logprob(logits_r_neg, m_neg).sum(dim=(1, 2, 3))

    ratio_pos = torch.exp(torch.clamp(lp_pos - lr_pos, -PPO_CLIP_VALUE, PPO_CLIP_VALUE))
    ratio_neg = torch.exp(torch.clamp(lp_neg - lr_neg, -PPO_CLIP_VALUE, PPO_CLIP_VALUE))

    adv_pos = torch.relu(adv)
    adv_neg = torch.relu(-adv)

    clipped_pos = torch.clamp(ratio_pos, 1 - eps, 1 + eps) * adv_pos
    clipped_neg = torch.clamp(ratio_neg, 1 - eps, 1 + eps) * adv_neg

    obj_pos = torch.minimum(ratio_pos * adv_pos, clipped_pos)
    obj_neg = torch.minimum(ratio_neg * adv_neg, clipped_neg)

    loss = -(obj_pos.mean() - obj_neg.mean())
    return loss


def get_warmup_cosine_lr(step: int, total_steps: int) -> float:
...
def evaluate_with_ema(policy, ema: ModelEMA, loader):
...
def main():
    ...
    ap.add_argument('--eval_only', action='store_true', help='仅评估EMA，不训练')
    args = ap.parse_args()
    ...
    if args.eval_only:
        iou_v, f1_v, p_v, r_v = evaluate_with_ema(policy, ema, val_loader)
        print(f"[OFFICIAL/VAL]  IoU={iou_v:.4f} F1={f1_v:.4f} P={p_v:.4f} R={r_v:.4f}")
        iou_t, f1_t, p_t, r_t = evaluate_with_ema(policy, ema, test_loader)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch in train_loader:
            ...
            m_pos, m_neg, adv = make_preference_masks(prob_p.detach(), prob_r.detach(), gt, th=0.5)
            ppo_loss = ppo_preference_loss(logits_p, logits_r.detach(), m_pos, m_neg, adv)
